...
- 修复官方“今日”日桶已经存在但刷新延迟时，旧值覆盖本机实时 `token_count` 的问题；当天展示现在使用官方与本机实时统计中更完整的值，同时继续以官方数据作为历史口径。
- 清理项目主页和仓库中的上游旧截图、公众号二维码、交流群二维码及相关宣传文案。

## 1.2.0 - 2026-07-17

- 为 `GPTcodex_U` 生成并替换统一品牌图标：程序坞采用白底、Codex 蓝紫色系的 3D `G > _` 图标，状态栏采用同形纯白单色图标。
- 应用对外显示名称统一调整为 `GPTcodex_U`，保留原 Bundle ID、缓存目录和可执行文件名以兼容既有设置与数据。
- 放大并加粗状态栏重置倒计时（如 `5d`），同时保持图标与文字居中对齐。
- 状态栏线性进度试用偏深的系统绿色填充，百分比改为带轻微深色阴影的加粗白字。
- 状态栏线性进度改为白色填充与半透明白灰底轨，百分比保留加粗黑字，在彩色桌面菜单栏上获得更稳定的对比度。
- 官方当天用量尚未发布时，使用排除 subagent 后的本机 `token_count` 事件实时补齐“今日”；官方日桶出现后自动切回官方值。
- 提高状态栏单额度百分比的字号与字重，并增加高对比胶囊底色和细描边，避免进度边界穿过数字时降低可读性。
- 将 Codex 累计、今日、近 7 天、月度与每日趋势统一到 app-server 官方 `account/usage` 数据，确保与 Codex 统计页一致；本机事件仅用于拆分和金额估算，并排除 subagent 重复计数。
- 加长状态栏单额度进度条，剩余模式的紫色填充改为从左侧开始，保留可见的有色底轨。
- 重新校验 Codex `token_count` 精细统计与本机线程口径，沿用 v1.1.1 的累计计数回退/重置去重逻辑，并升级安装时重建旧版统计缓存。
- Token 数值统一按中文数量级展示：不足一万显示原数，达到一万使用“万”，达到一亿使用“亿”；API 等效金额不再使用 K/M 缩写。
- 提高状态栏单额度进度条底层轨道的配色可见度，未填充区也保留清晰的有色轨道。
