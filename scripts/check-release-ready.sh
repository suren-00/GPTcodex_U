...
grep -q "GPTcodex_U-${VERSION}-mac-arm64.dmg" README.md || { echo "README.md artifact examples are stale" >&2; exit 1; }
grep -q "GPTcodex_U-${VERSION}-mac-arm64.dmg" README.en.md || { echo "README.en.md artifact examples are stale" >&2; exit 1; }
...
for arch in arm64 x86_64; do
  dmg="dist/GPTcodex_U-${VERSION}-mac-${arch}.dmg"
  checksum="${dmg}.sha256"
